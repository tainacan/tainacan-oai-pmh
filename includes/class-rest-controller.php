...
class REST_Controller {
    ...
    public function register_routes() {
        register_rest_route($this->namespace, '/oai', [
            'methods' => ['GET', 'POST'],
    ...
    public function handle_request(\WP_REST_Request $request) {
        ...
        $valid_verbs = ['Identify', 'ListMetadataFormats', 'ListSets', 'ListRecords', 'ListIdentifiers', 'GetRecord'];
    ...
    private function parse_list_params($xml, $params) {
        ...
        if (!empty($params['from'])) $query['from'] = $this->parse_date($params['from']);
        if (!empty($params['until'])) $query['until'] = $this->parse_date($params['until']);
    ...
    private function respond($xml, $verb, $params, $start) {
        ...
        $this->logger->log("OAI Request: $verb", 'info', ['verb' => $verb, 'response_time' => $time]);
        
        $response = new \WP_REST_Response();
        $response->set_headers(['Content-Type' => 'text/xml; charset=utf-8']);
