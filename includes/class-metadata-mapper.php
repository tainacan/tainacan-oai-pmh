...
class Metadata_Mapper {
    ...
    public static function get_collection_metadata(int $collection_id): array {
        ...
        $result = [];
        if (!is_array($metadata)) return $result;
        ...
        foreach ($metadata as $metadatum) {
            ...
            $is_core = in_array($type, ['Tainacan\\Metadata_Types\\Core_Title', 'Tainacan\\Metadata_Types\\Core_Description'], true);
    ...
    public static function build_mapping_rows(int $collection_id, array $source_fields): array {
        ...
        $source_by_name = [];
        foreach ($source_fields as $f) $source_by_name[$f['name']] = $f;
        ...
        foreach ($source_fields as $f) {
            if (isset($standard[$f['name']])) continue;
            $rows[] = [
                'name' => $f['name'],
