...
class Harvester {
	...
	public const SCHEDULES = array( 'hourly', 'twicedaily', 'daily', 'weekly' );
	...
	public static function register_hooks(): void {
		...
		add_action( self::CRON_HOOK, array( self::class, 'run_via_cron' ), 10, 1 );
	}
	...
	private function sanitize_input( array $args, $existing = null ) {
		...
		if ( $label === '' ) {
			return new \WP_Error( 'missing_label', __( 'Label is required.', 'tainacan-oai-pmh' ) );
		}
		if ( $url === '' ) {
			return new \WP_Error( 'missing_url', __( 'Source URL is required.', 'tainacan-oai-pmh' ) );
		}
		if ( $collection_id <= 0 ) {
			return new \WP_Error( 'missing_collection', __( 'Collection is required.', 'tainacan-oai-pmh' ) );
		}
		if ( ! in_array( $schedule, array_merge( self::SCHEDULES, array( 'paused' ) ), true ) ) {
		...
		if ( ! is_array( $mapping ) ) {
			$mapping = $existing ? maybe_unserialize( $existing->metadata_mapping ) : array();
			if ( ! is_array( $mapping ) ) {
				$mapping = array();
			}
		}
	...
	public function schedule( int $id ): void {
		$source = $this->get( $id );
		if ( ! $source || $source->schedule === 'paused' || empty( $source->is_active ) ) {
			return;
		}
		if ( ! in_array( $source->schedule, self::SCHEDULES, true ) ) {
			return;
		}
		...
		wp_schedule_event( time() + 60, $source->schedule, self::CRON_HOOK, array( $id ) );
	}
	...
	public function next_scheduled( int $id ): ?int {
		$ts = wp_next_scheduled( self::CRON_HOOK, array( $id ) );
		return $ts ?: null;
	}
	...
	public static function unschedule_all(): void {
		...
		foreach ( $cron as $timestamp => $hooks ) {
			...
			foreach ( $hooks[ self::CRON_HOOK ] as $event ) {
				wp_unschedule_event( $timestamp, self::CRON_HOOK, $event['args'] ?? array() );
			}
		}
	}
	...
	public function run( int $id ) {
		...
		$log_lines   = array();
		$log_lines[] = $this->log_line(
			'INFO',
			'run.start',
			sprintf(
				'Started harvest %s (from=%s, set=%s, collection=%d)',
				$source->label,
				$config['from'] ?: '(full)',
				$config['set_spec'] ?: '(all)',
				(int) $source->collection_id
			)
		);
		$log_lines[] = $this->log_line(
			'INFO',
		...
		$log_lines[] = $this->log_line(
			...
			'run.diff',
			sprintf(
				'created=%d, updated=%d, skipped=%d, failed=%d, deleted=%d',
				$stats['created'],
				$stats['updated'],
				$stats['skipped'],
				$stats['failed'],
