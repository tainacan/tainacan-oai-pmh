...
class Validator {
    ...
    private function test_identify() {
        $response = $this->request(['verb' => 'Identify']);
        $test = ['name' => 'Identify', 'description' => __('Checks Identify response.', 'tainacan-oai-pmh'), 'status' => 'passed', 'details' => []];
        
        if ($response['error']) {
            $test['status'] = 'failed';
            $test['details'][] = 'Error: ' . $response['error'];
        } else {
            foreach (['repositoryName', 'baseURL', 'protocolVersion', 'adminEmail', 'earliestDatestamp', 'deletedRecord', 'granularity'] as $field) {
    ...
    private function test_list_metadata_formats() {
        $response = $this->request(['verb' => 'ListMetadataFormats']);
        $test = ['name' => 'ListMetadataFormats', 'description' => __('Checks oai_dc support.', 'tainacan-oai-pmh'), 'status' => 'passed', 'details' => []];
    ...
    private function test_list_sets() {
        $response = $this->request(['verb' => 'ListSets']);
        $test = ['name' => 'ListSets', 'description' => __('Checks sets/collections.', 'tainacan-oai-pmh'), 'status' => 'passed', 'details' => []];
    ...
    private function test_list_records() {
        $response = $this->request(['verb' => 'ListRecords', 'metadataPrefix' => 'oai_dc']);
        $test = ['name' => 'ListRecords', 'description' => __('Checks record listing.', 'tainacan-oai-pmh'), 'status' => 'passed', 'details' => []];
    ...
    private function test_list_identifiers() {
        $response = $this->request(['verb' => 'ListIdentifiers', 'metadataPrefix' => 'oai_dc']);
        $test = ['name' => 'ListIdentifiers', 'description' => __('Checks identifier listing.', 'tainacan-oai-pmh'), 'status' => 'passed', 'details' => []];
    ...
    private function test_get_record() {
        $test = ['name' => 'GetRecord', 'description' => __('Checks single record retrieval.', 'tainacan-oai-pmh'), 'status' => 'passed', 'details' => []];
        
        // Get an identifier first
        $list = $this->request(['verb' => 'ListIdentifiers', 'metadataPrefix' => 'oai_dc']);
        if (preg_match('/<identifier>([^<]+)<\/identifier>/', $list['body'], $matches)) {
            $response = $this->request(['verb' => 'GetRecord', 'identifier' => $matches[1], 'metadataPrefix' => 'oai_dc']);
    ...
    private function test_error_handling() {
        $test = ['name' => 'Error Handling', 'description' => __('Checks OAI-PMH error responses.', 'tainacan-oai-pmh'), 'status' => 'passed', 'details' => []];
        
        $response = $this->request(['verb' => 'InvalidVerb']);
        ...
        $response = $this->request(['verb' => 'ListRecords']);
